...
#else
    const char* pszModule = "donationcoin";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "donationcoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No donationcoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override donationcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Donationcoind.pid"));
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 35 * 60) // Donationcoin: changed maximum adjust to 35 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Donationcoin will not work properly.");
...
// Format the subversion field according to BIP 14 spec (https://en.donationcoin.it/wiki/BIP_0014)
