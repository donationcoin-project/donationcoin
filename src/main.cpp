...
uint256 hashGenesisBlock("0x56909b681667b2e089791a65f1d0953b5fe994a12a039b0057b48187e39040cc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Donationcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Donationcoin Signed Message:\n";
...
{
    // Donationcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 90000000 * 20  / 100 / 50;
     }

    nSubsidy >>= (nHeight / 720000);


    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 96 * 60 * 60;
static const int64 nTargetSpacing = 1 * 240;
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Donationcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
/* current difficulty formula, donationcoin - gravity well */
...
if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin) { return bnProofOfWorkLimit.GetCompact(); }
    ...
    for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
            ...
            if (BlockReading->nHeight > 22000) LatestBlockTime = BlockReading->GetBlockTime();
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    printf("Into testnet @ block # %d Diffmode: %d\n", pindexLast->nHeight+1, DiffMode );
        ...
        if (pindexLast->nHeight+1 >= 22000) { DiffMode = 2; }
}
...
void ThreadScriptCheck() {
    RenameThread("donationcoin-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Donationcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Donationcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x56909b681667b2e089791a65f1d0953b5fe994a12a039b0057b48187e39040cc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1397146716;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086721889;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Donationcoin: increase each by adding 2 to donationcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DonationcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DonationcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DonationcoinMiner(CWallet *pwallet)
{
    printf("DonationcoinMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("donationcoin-miner");
    ...
    try { loop {
        ...
        printf("Running Donationcoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DonationcoinMiner terminated\n");
        throw;
    }
}

void GenerateDonationcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DonationcoinMiner, pwallet));
}
