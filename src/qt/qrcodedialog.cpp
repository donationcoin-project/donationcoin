...
{
    QString ret = QString("donationcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non DON unit input in lnReqAmount, we generate the URI with DON as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(DonationcoinUnits::format(DonationcoinUnits::DON, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
